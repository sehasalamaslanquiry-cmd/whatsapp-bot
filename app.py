...
def get_ai_response(user_text):
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system", 
                "content": "أنت مساعد المهندس سامي المجيدي. جاوب باللهجة التعزية بذكاء. تأكد من كتابة الكلمات العربية والحروف كاملة وبوضوح دون نقص. إذا سألك أحد عن (إجازات مرضية) أو (أي شغل)، قله: 'أبشر يا غالي، سامي بيشتغلك على طول، بس ارسل بياناتك كاملة وما عليك'. إذا سألو عن (السعر أو التكلفة)، قله: 'السعر كالعادة، اتفق أنت وسامي '، واسأله: 'تشتي رقمه المباشر؟'. إذا وافق، أعطيه هذا الرقم: 967739704861. إذا سألك شيء ما تعرفوش، قل: 'والله ما بش معي خبر عليه، اسأل سامي أو راسله على رقمه المباشر'."
            },
            {"role": "user", "content": user_text}
        ],
        "temperature": 0.5,  # هذا الرقم يجعل الرد متزناً ولا يخطئ في الحروف
        "max_tokens": 500    # يضمن أن الجملة لا تنقطع في المنتصف
    }
    try:
        response = requests.post(url, headers=headers, json=payload, timeout=10)
        res_data = response.json()
        return res_data['choices'][0]['message']['content'] if response.status_code == 200 else "أهلاً يا غالي."
    except:
        return "عذراً يا طيب، حصل تشويش بسيط."
